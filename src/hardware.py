...
class KoradKD3305PSource:
    ...
    def __init__(self, source_address, dc_field_conversion_factor):
        ...
        self.output(False, channel=1)
        self.set_voltage(20, channel=1)
        self.output(False, channel=2)
    ...
    def read_values(self, channel):
        ...
        # The source will return something liek 119700020 which translates to:
        # U = 11.97 V, I = 0.00 A and it is in C.V. mode (constant voltage)
        self.source.write(str.encode("VOUT{0}?\n".format(channel)))
        raw_voltage = self.source.readline(7).decode()
        try:
            voltage = float(raw_voltage)
        except:
            cf.log_message("Couldn't read out voltage")
        ...
        time.sleep(0.2)
        self.source.write(str.encode("IOUT{0}?\n".format(channel)))
        raw_current = self.source.read(7).decode()
        try:
            current = float(raw_current)
        except:
            cf.log_message("Couldn't read out current")
    ...
    def set_voltage(self, voltage, channel):
        ...
        try:
            self.source.write(str.encode("VSET{0}:{1}\n".format(channel, voltage)))
        except SerialTimeoutException as err:
            cf.log_message(err)
            time.sleep(0.2)
            self.source.write(str.encode("VSET{0}:{1}\n".format(channel, voltage)))
    ...
    def set_current(self, current, channel):
        ...
        self.source.write(str.encode("ISET{0}:{1}\n".format(channel, current)))
        time.sleep(0.2)

    def set_magnetic_field(self, magnetic_field, channel):
        """
        Function that converts a value for a magnetic field to a current that
        can be set on the source
        """
    ...
    def adjust_magnetic_field(
        ...
        while True:
            ...
            # Set the voltage to that value (rounded to the accuracy of the
            # source)
            self.set_voltage(round(pid_voltage, 2), channel)
    ...
    def output(self, state, channel, slow=False):
        """
        Activate or deactivate output:
        state = True: output on
        state = False: output off
        ...
        if slow:
            ...
            self.source.write(str.encode("OUT{0}:{1}\n".format(channel, int(state))))
            time.sleep(1)

            self.set_voltage(round(voltage / 2, 2), channel)
            time.sleep(0.2)
            self.set_voltage(round(voltage, 2), channel)
            time.sleep(0.2)
        else:
            try:
                self.source.write(
        ...
        # Set the state variable
        if channel == 1:
            self.dc_output_state = state
        elif channel == 2:
            self.hf_output_state = state
